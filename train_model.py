...
num_records = 500
# Limit coordinates strictly within city bounds to avoid ocean
latitudes = np.random.uniform(17.710, 17.748, num_records)
longitudes = np.random.uniform(83.310, 83.340, num_records)
...
X = df[['Hour', 'Day', 'Weekday', 'Traffic_Level']]
...
# ---------------- Cell 5: KMeans clustering (zones) ----------------
...
# Snap centroids to nearest actual data point to avoid ocean
for idx, row in zone_centers.iterrows():
    cluster_points = df[df['Zone'] == row['Zone']]
    nearest_point_idx = ((cluster_points[['Latitude','Longitude']] - row[['Latitude','Longitude']]).pow(2).sum(axis=1)).idxmin()
    zone_centers.loc[idx, ['Latitude','Longitude']] = cluster_points.loc[nearest_point_idx, ['Latitude','Longitude']]
...
# ---------------- Cell 7: Generate emergency heatmap ----------------
df['Predicted_Emergency'] = model.predict(X)  # Use model predictions
